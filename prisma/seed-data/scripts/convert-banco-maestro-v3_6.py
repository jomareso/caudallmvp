...
# "Banco_Maestro_Caudall_v3_6.xlsx" (reorganización de la fundadora: pasó
# de 8 pestañas planas a 4 con secciones — 00_ARQUITECTURA es solo
# documentación visual, no se carga — y agregó la estructura primary/
# confirmatory/reserve para los ítems conductuales, con los reserve
# marcados DRAFT). Reemplaza a convert-banco-maestro-v3_1.py, que se deja
# como registro de la conversión anterior. Requiere openpyxl. No corre en
# CI — paso manual de una sola vez por revisión del Excel (ver README.md).

import sys
import json
import openpyxl

SRC = sys.argv[1] if len(sys.argv) > 1 else 'Banco_Maestro_Caudall_v3_6.xlsx'
...
qrows = [r for r in ws_qa.iter_rows(min_row=26, max_row=ws_qa.max_row, values_only=True) if r[0]]
qa_scenarios = []
for r in qrows:
    qa_scenarios.append({
        'code': norm(r[qidx['ID']]),
