...
export default async function CorreoActualizadoPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <EmployeeTopBar />
      {/* max-w-lg, ícono/texto más grandes (no max-w-sm, w-14/text-lg/text-sm):
      ...
      <main className="flex-1 flex flex-col items-center p-6 pt-16">
        <div className="w-full max-w-lg text-center">
          <div className="w-20 h-20 rounded-full bg-ok/10 text-ok flex items-center justify-center text-4xl mx-auto mb-4">
            ✓
          </div>
          <h1 className="text-3xl font-medium text-quartz mb-3">{t('title')}</h1>
          <p className="text-lg text-nickel mb-6">{t('subtitle')}</p>
          <Link href="/perfil" className="inline-block bg-yale text-white rounded-lg py-2.5 px-6 text-base">
            {t('ctaBack')}
          </Link>
        </div>
      </main>
